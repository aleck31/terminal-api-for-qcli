...
from .websocket_client import WebSocketMessage
...
class TerminalAPIClient:
    ...
    def execute_command(self, command: str, timeout: float = 30.0) -> Generator[Tuple[str, str, Dict[str, Any]], None, None]:
        ...
        """
        yield from self._execute_command(command, timeout)

    def _execute_command(self, command: str, timeout: float = 30.0) -> Generator[Tuple[str, str, Dict[str, Any]], None, None]:
        """使用连接管理器执行命令"""
        # 开始命令执行
        output_type, content, metadata = self.message_processor.start_command_execution(
            command)
            ...
            while time.time() - start_time < timeout:
                ...
                if message_event.wait(timeout=1.0):
                    ...
                    while messages:
                        ...
                        # 处理消息
                        result = self.message_processor.process_websocket_message(
                            legacy_message)
                        if result:
